...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="bg-gradient-to-r from-blue-700 to-indigo-600 px-8 py-6">
            <h1 class="text-xl font-bold text-white">Create a Post</h1>
            <p class="text-blue-200 text-sm mt-1">
                Share something with the alumni community.
            </p>
        </div>

        <form method="POST" action="{{ route('posts.store') }}" class="p-8">
            @csrf
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Content
                    <span class="text-gray-400 font-normal">(max 3000 characters)</span>
                </label>
                <textarea name="body" rows="6"
                          placeholder="Share your update, opportunity, or story..."
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5
                                 text-sm bg-gray-50 focus:outline-none focus:ring-2
                                 focus:ring-blue-500 resize-none">{{ old('body') }}</textarea>
            </div>
